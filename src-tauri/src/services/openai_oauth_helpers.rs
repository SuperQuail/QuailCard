...
impl Drop for OpenAiAccess {
    /// 被停止的生成 Future 释放时也清零短期访问令牌。
    fn drop(&mut self) {
...
/// 将持久化凭据裁剪为模型请求所需字段。
pub(super) fn to_access(mut credential: OpenAiOAuthCredential) -> OpenAiAccess {
    OpenAiAccess {
        access_token: std::mem::take(&mut credential.access_token),
        account_id: credential.account_id.take(),
    }
}
...
pub(super) async fn read_oauth_credential(
    ...
    let serialized = Zeroizing::new(vault.get_credential(storage, secret_ref).await?);
