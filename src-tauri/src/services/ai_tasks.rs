...
impl AppServices {
    ...
    pub async fn evaluate_answer(
        ...
        let tool = evaluation_tool().definition();
    ...
    async fn call_configured_tool(
        ...
        match config.auth_type.as_deref() {
            ...
            Some("api_key") => {
                let api_key = Zeroizing::new(self.load_api_key(storage, config).await?);
    ...
    pub(super) async fn call_configured_multi_tool(
        ...
        self.call_configured_multi_tool_streaming(storage, config, request, None)
            .await
    }

    /// 带可见文本增量的多工具调用；delta 只接收正文，不接收思考。
    pub(super) async fn call_configured_multi_tool_streaming<'a>(
        &'a self,
        storage: &'a Storage,
        config: &'a ProviderConfig,
        request: MultiToolRequest<'a>,
        delta: Option<&'a (dyn Fn(&str) + Send + Sync)>,
    ) -> Result<ToolCallBatch, CommandError> {
        match config.auth_type.as_deref() {
            Some("openai_oauth") => {
                let access = self.load_openai_access(storage, config).await?;
                self.ai
                    .call_openai_oauth_multi_tool_with_delta(
                        config,
                        &access.access_token,
                        access.account_id.as_deref(),
                        request,
            ...
            Some("api_key") => {
                let api_key = Zeroizing::new(self.load_api_key(storage, config).await?);
                self.ai
                    .call_multi_tool_with_delta(config, &api_key, request, delta)
                    .await
            }
            _ => Err(CommandError::new(
                "PROVIDER_CREDENTIAL_MISSING",
