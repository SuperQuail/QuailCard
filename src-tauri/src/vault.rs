...
impl EncryptedVault {
    ...
    pub async fn initialize(&self, database: &Database) -> Result<(), CommandError> {
        ...
        if database.get_vault_envelope().await?.is_some() {
            return Ok(());
        }
        let envelope = encrypt_new_default(&VaultPayload::empty())?;
        database.save_vault_envelope(&envelope).await
    }
    ...
    pub async fn get_credential(
        ...
        let envelope = require_envelope(database).await?;
    ...
    /// 在内存载荷中写入新凭据、移除旧引用并生成待事务提交的新密文记录。
    pub async fn prepare_set_credential(
        ...
        let current = require_envelope(database).await?;
    ...
    pub async fn unlock(
        ...
        if envelope.protection_mode != MODE_PASSWORD {
            return self.status(database).await;
        }
    ...
    pub async fn set_password(
        ...
        database.save_vault_envelope(&next).await?;
        *self.unlocked_password_key.write().await = Some(key);
        self.status(database).await
    }
    ...
    pub async fn reset(
        ...
        let _guard = self.lock_operations().await;
        let envelope = encrypt_new_default(&VaultPayload::empty())?;
        database.reset_credential_vault(&envelope).await?;
        *self.unlocked_password_key.write().await = None;
        self.status(database).await
    }
...
/// 查询必须存在的唯一保险库密文记录。
async fn require_envelope(database: &Database) -> Result<VaultEnvelope, CommandError> {
    database
        .get_vault_envelope()
        .await?
...
mod tests {
    ...
    /// 创建已经初始化默认保险库的内存数据库。
    async fn test_vault() -> (Database, EncryptedVault) {
    ...
    #[tokio::test]
    /// 默认保护可以读写凭据且 SQLite 密文不包含原文。
    async fn default_mode_encrypts_credentials() {
        let (database, vault) = test_vault().await;
        let credential = "sk-sensitive-value";
        let envelope = vault
            .prepare_set_credential(&database, "secret-1", credential, None)
            .await
            .expect("加密凭据失败");
        assert!(!envelope
            .ciphertext
    ...
    async fn encryption_uses_fresh_nonce() {
        let (database, vault) = test_vault().await;
        let first = vault
            .prepare_set_credential(&database, "secret-1", "value", None)
            .await
            .expect("第一次加密失败");
        let second = vault
            .prepare_set_credential(&database, "secret-1", "value", None)
    ...
    async fn password_mode_requires_unlock() {
        let (database, vault) = test_vault().await;
        let envelope = vault
            .prepare_set_credential(&database, "secret-1", "oauth-token", None)
            .await
            .expect("加密凭据失败");
        database
            .save_vault_envelope(&envelope)
            ...
            .expect("保存保险库失败");
        vault
            .set_password(&database, "correct-password")
            .await
            .expect("设置密码失败");
        vault.lock(&database).await.expect("锁定保险库失败");
        assert_eq!(
            vault
                .get_credential(&database, "secret-1")
                .await
        ...
        vault
            .unlock(&database, "correct-password")
            .await
            .expect("正确密码应解锁");
        assert_eq!(
            vault
        ...
        let status = vault
            .remove_password(&database)
            .await
            .expect("移除密码失败");
        assert_eq!(status.protection_mode, MODE_DEFAULT);
        vault.lock(&database).await.expect("默认模式锁定调用失败");
        assert_eq!(
            vault
                .get_credential(&database, "secret-1")
                .await
    ...
    async fn reset_clears_provider_credentials() {
        ...
        vault
            ...
            .expect("设置密码失败");
        vault.lock(&database).await.expect("锁定保险库失败");
        let status = vault.reset(&database).await.expect("重置保险库失败");
        assert_eq!(status.protection_mode, MODE_DEFAULT);
        ...
        assert!(vault.get_credential(&database, "secret-1").await.is_err());
    }
}
