...
use crate::{
    ...
    error::CommandError,
    models::{
        BootstrapData, ConnectionTestResult, DataFolderTarget, DataLocations, NoteFile,
        NoteSummary, OpenAiLoginMode, OpenAiLoginStart, OpenAiLoginStatus, ProviderInput,
        ProviderSummary, SaveNoteInput, StudyStats, VaultStatus,
    },
    paths,
...
pub async fn open_vault(
    agent_tasks: State<'_, crate::services::agent_tasks::AgentTasks>,
    vault: State<'_, VaultState>,
    storage: State<'_, Storage>,
    path: String,
) -> Result<String, CommandError> {
    agent_tasks.ensure_idle()?;
    let root = vault.set_root(PathBuf::from(path))?;
    let files = vault.scan()?;
    storage.open_vault(&root, &files).await?;
    // 对外只暴露简化后的路径：canonicalize 产生的 `\\?\` 前缀不应进入设置或界面。
    let display = paths::simplified(&root);
    storage.record_recent_vault(&display).await?;
    Ok(display)
}
...
pub async fn get_vault_path(vault: State<'_, VaultState>) -> Result<Option<String>, CommandError> {
    Ok(vault.root()?.map(paths::simplified))
}
...
pub async fn get_bootstrap_data(
    ...
    Ok(BootstrapData {
        ...
        vault_path: vault.root()?.map(paths::simplified),
...
pub async fn get_data_locations(
    ...
    Ok(DataLocations {
        cards_dir: storage.cards_dir().map(paths::simplified),
