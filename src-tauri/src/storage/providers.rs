...
use crate::{
    error::CommandError,
    models::{
        ProviderConfig, ProviderInput, ProviderModel, ProviderSummary, VaultEnvelope,
        MAX_MODEL_OUTPUT_TOKENS, MAX_PROVIDER_MODELS, MIN_CONTEXT_WINDOW_TOKENS,
    },
};
...
impl Storage {
    ...
    pub async fn save_provider_config(
        ...
        let record = ProviderRecord {
            ...
            // 写完整：目录按前端提交内容落盘，id 为空的条目在此丢弃。
            models: sanitize_models(&input.models),
...
/// 读出模型目录：目录为空但 model 非空时合成单条，旧 providers.toml 行为不变。
///
/// 契约：返回的条目已去空白且 name 回退为 id，前端拿到的目录永远可直接展示；
/// 目录非空时以目录为准，model 字段只用于匹配 active_model。
fn models_of(record: &ProviderRecord) -> Vec<ProviderModel> {
    let mut models = sanitize_models(&record.models);
    let legacy = record.model.trim();
    if models.is_empty() && !legacy.is_empty() {
...
/// 规范化模型目录：去空白并在 name 缺省时回退 id，丢弃 id 为空的条目。
///
/// 契约：这里只做规范化，不做去重与数量限制——那是 validate_provider 的职责，
/// 避免"被拒绝的输入"与"被裁剪的读出"互相掩盖。
fn sanitize_models(models: &[ProviderModel]) -> Vec<ProviderModel> {
    models
        .iter()
        .map(ProviderModel::normalized)
...
fn validate_provider(input: &ProviderInput) -> Result<(), CommandError> {
    ...
    validate_model_catalog(&input.models)
}

/// 校验模型目录：条数上限、id 非空且不重复、两个上限的取值范围。
///
/// 契约：目录允许为空（老前端只传 model 时由读取方合成单条），但 model 字段
/// 本身仍必填；错误消息只说明规则，不回显用户输入内容。
fn validate_model_catalog(models: &[ProviderModel]) -> Result<(), CommandError> {
    if models.len() > MAX_PROVIDER_MODELS {
        return Err(CommandError::validation(format!(
            "模型目录最多 {MAX_PROVIDER_MODELS} 个模型"
        )));
    }
    ...
    for entry in models {
        ...
        if id.is_empty() {
            return Err(CommandError::validation("模型 ID 不能为空"));
        }
        if seen.contains(&id) {
            return Err(CommandError::validation("同一供应商的模型 ID 不能重复"));
        }
        seen.push(id);
        if entry
            .context_window
            .is_some_and(|window| window < MIN_CONTEXT_WINDOW_TOKENS)
        {
            return Err(CommandError::validation("模型上下文窗口不能小于 1024"));
        }
        if let Some(max_output_tokens) = entry.max_output_tokens {
            if max_output_tokens == 0 || max_output_tokens > MAX_MODEL_OUTPUT_TOKENS {
                return Err(CommandError::validation(
                    "模型单次最大输出必须在 1 到 200000 之间",
                ));
            }
        }
    }
