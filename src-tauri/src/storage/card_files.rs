...
#[cfg(test)]
#[path = "card_files_video_tests.rs"]
mod video_tests;
...
pub(super) fn load_card_files(root: &Path, state: &mut CardState) -> Result<(), CommandError> {
    ...
    while let Some(directory) = stack.pop() {
        for entry in std::fs::read_dir(&directory)? {
            ...
            let path = entry.path();
            if path.is_dir() {
                if directory == cards_dir.join("agent")
            ...
            if relative == "config.json"
                || relative.starts_with("agent/.")
                || !relative.to_lowercase().ends_with(".json")
                || is_video_internal_file(root, &path, &relative)
            {
                continue;
            }
...
/// 识别视频存储的固定布局；不能排除整个 video 目录，否则会隐藏同名笔记卡片。
fn is_video_internal_file(root: &Path, path: &Path, relative: &str) -> bool {
    let parts: Vec<_> = relative.split('/').collect();
    let reserved = match parts.as_slice() {
        ["video", "tasks", id, "task.json" | "transcript.json"] => {
            uuid::Uuid::parse_str(id).is_ok()
        }
        ["video", "pages", name] => name
            .strip_suffix(".json")
            .and_then(|name| name.rsplit_once('-'))
            .is_some_and(|(key, cid)| !key.is_empty() && cid.parse::<u64>().is_ok()),
        _ => false,
    };
    if !reserved || root.join(derive_note_path(relative)).is_file() {
        return false;
    }
    // 同名旧卡片可能已没有源笔记，仍按 cards 字段识别并保留严格的损坏保护。
    let is_card = std::fs::read(path)
        .ok()
        .and_then(|bytes| serde_json::from_slice::<serde_json::Value>(&bytes).ok())
        .is_some_and(|value| value.get("cards").is_some());
    !is_card
}
...
pub(super) fn persist_cards(
    ...
    ensure_card_path_in_vault(root, &path)?;
    if cards.is_empty() {
        if path.exists() {
...
/// 原子写之前校验已有目标或最近祖先，拒绝数据目录符号链接把写入导向 Vault 外。
fn ensure_card_path_in_vault(root: &Path, path: &Path) -> Result<(), CommandError> {
    let root = root.canonicalize()?;
    let ancestor = path
        .ancestors()
        .find(|candidate| candidate.exists())
        .ok_or_else(|| CommandError::validation("卡片路径无效"))?;
    if !ancestor.canonicalize()?.starts_with(&root) {
        return Err(CommandError::validation("卡片路径超出 Vault 范围"));
    }
    Ok(())
}
