...
impl CardStore {
        ...
        let note_card = {
            ...
            let record = CardRecord {
                ...
                source_ref: input
                    .source_ref
                    .clone()
                    .or_else(|| existing.map(|index| cards[index].source_ref.clone()))
                    .unwrap_or_default(),
                source: input
                    .source
                    .clone()
                    .or_else(|| existing.and_then(|index| cards[index].source.clone())),
    ...
    pub(crate) fn rename_note_paths(
        ...
        for key in affected {
            ...
            let cards = state.notes.get(&key).cloned().unwrap_or_default();
            let source = root.join(card_relative_path(&key)?);
            // 镜像信封内的 notePath 也必须改写，否则重扫后卡片会回到旧笔记。
            persist_cards(&root, &new_key, &cards)?;
            if source.exists() {
                std::fs::remove_file(&source)?;
                cleanup_empty_parents(&root, source.parent());
            }
            state.notes.remove(&key);
...
/// 校验单张卡片的通用约束。
pub(super) fn validate_card_input(input: &CardInput) -> Result<(), CommandError> {
