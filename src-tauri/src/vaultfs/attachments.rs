...
impl VaultState {
    /// 保存生成式配图到附件目录，返回笔记中可直接使用的相对路径。
    ///
    /// 只允许图片扩展名，写入前校验大小；同名文件追加序号，绝不覆盖。
    pub fn save_generated_image(
        ...
        if bytes.is_empty() || bytes.len() > 12 * 1024 * 1024 {
            return Err(CommandError::validation("配图大小必须在 12MiB 以内"));
        }
        let root = self.require_root()?;
        let folder = normalize_attachment_folder(&self.get_config()?.attachment_folder)?;
        let note_parent = Path::new(note_folder).to_path_buf();
        let stem = safe_file_stem(file_name)?;
        let directory = create_contained_directories(&root, &folder)?;
        let mut target = directory.join(format!("{stem}.jpg"));
        let mut index = 2;
        while target.exists() {
            target = directory.join(format!("{stem}-{index}.jpg"));
            index += 1;
        }
        ensure_contained(&root, &target)?;
        write_new_file(&target, bytes, "保存配图失败")?;
        let note_parent_absolute = root.join(&note_parent);
        Ok(relative_markdown_path(&note_parent_absolute, &target))
    }
}
