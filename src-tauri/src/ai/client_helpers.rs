...
use super::super::{ProviderProtocol, ToolDefinition};
...
/// 判断无工具响应是否仍有重试额度；重试原因由每次请求的诊断行统一记录。
pub(super) fn retry_missing_tool(retry: usize, error: &CommandError) -> bool {
    error.code == "PROVIDER_TOOL_NOT_CALLED" && retry < MAX_MISSING_TOOL_RETRIES
}
...
pub(crate) fn completion_endpoint(
    base_url: &str,
