...
async function importAndInsert(
  ...
  for (const file of files) {
    ...
    if (view.state.readOnly || currentNotePath() !== originNotePath) {
  ...
  if (!markdown.length || view.state.readOnly || currentNotePath() !== originNotePath) {
    clearMarkers(view, [markerIds.from, markerIds.to]);
    return;
  }
