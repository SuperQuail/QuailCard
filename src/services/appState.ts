...
import { leaveAgentVault } from "./stores/agentStore";
...
export async function openVault(path: string): Promise<void> {
  await leaveAgentVault();
  if (noteStore.noteOperationBusy.value) return;
  noteStore.noteOperationBusy.value = true;
  try {
    await noteStore.notePersistence.flushAll();
    ...
    uiStore.errorMessage.value = backend.resolveErrorMessage(error);
  } finally { noteStore.noteOperationBusy.value = false; }
}
...
export async function leaveVault(): Promise<void> {
  ...
  try {
    ...
    cardStore.clearActiveCards();
    await vaultStore.loadDataLocations();
  } catch (error) { uiStore.errorMessage.value = backend.resolveErrorMessage(error); }
  finally { noteStore.noteOperationBusy.value = false; }
}

/** 窗口聚焦时重扫 Vault：刷新笔记列表并重载当前笔记内容。 */
export async function rescanVault(): Promise<void> {
  if (!vaultStore.vaultPath.value || noteStore.noteOperationBusy.value) {
    return;
  }
  noteStore.noteOperationBusy.value = true;
  try {
    ...
    noteStore.noteOperationBusy.value = false;
  }
  await noteStore.reloadActiveContent().catch((error) => { uiStore.errorMessage.value = backend.resolveErrorMessage(error); });
}

/** 打开笔记：加载正文（含索引重建）后加载其卡片。 */
export async function selectNote(path: string): Promise<void> {
  await noteStore.selectNote(path);
  if (noteStore.activeNotePath.value === path) await cardStore.loadActiveCards(path);
}

/** 标签关闭后的空工作区同步清除卡片；调用者须先完成当前草稿保存。 */
