...
/** AI 判定回答。 */
export async function evaluateAnswer(cardId: string, userAnswer: string, expectedVersion: number, operationId = crypto.randomUUID()): Promise<AiEvaluationResult> {
  return backend.evaluateAnswer(cardId, userAnswer, expectedVersion, operationId);
}
