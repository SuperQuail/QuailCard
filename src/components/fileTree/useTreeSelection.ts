...
/** 选择修饰键：真实点击事件与程序化选择（拖拽起手）共用同一契约。 */
...
export function useTreeSelection(rows: ComputedRef<TreeRow[]>) {
  ...
  /** 行选择逻辑：Shift 范围、Ctrl 切换、普通单选。 */
  function selectRow(row: TreeRow, modifiers: SelectionModifiers): void {
    const key = rowKey(row);
    const keys = rows.value.map(rowKey);
    if (modifiers.shiftKey && selectionAnchor.value) {
