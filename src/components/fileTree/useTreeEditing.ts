...
export function useTreeEditing() {
  ...
  async function commitRename(onRename: (kind: "folder" | "note", oldPath: string, newPath: string) => Promise<void>): Promise<void> {
    const state = renaming.value;
    if (!state || renameBusy.value) {
      return;
    }
    const value = state.value.trim();
    if (!value) { renameError.value = "名称不能为空"; return; }
    const nextPath = renamePath(state.key, value, state.kind === "folder");
    if (nextPath === state.key) { renaming.value = null; return; }
    renameBusy.value = true;
    renameError.value = "";
    try {
      await onRename(state.kind, state.key, nextPath);
      renaming.value = null;
    } catch (error) {
      renameError.value = error instanceof Error ? error.message : "重命名失败，请重试";
