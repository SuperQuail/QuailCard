...
const emit = defineEmits<{
  close: [];
  "reselect-source": [{ kind: CardKind; front: string; back: string; detail: string; example: string; rubric: string }];
...
/** 动态弹窗挂载后聚焦问题输入，不依赖浏览器处理 autofocus。 */
...
function submit(): void {
  if (props.busy || !front.value.trim() || !back.value.trim()) {
    return;
  }
  emit("save", {
...
<template>
  <div class="overlay-backdrop flex items-start justify-center pt-24" @click.self="!busy && emit('close')">
    <div class="modal-panel w-full max-w-[420px] p-5">
      <header class="mb-4 flex items-center justify-between">
        <h2 class="text-[14px] font-semibold">{{ editingId ? "编辑卡片" : "拆成卡片" }}</h2>
        <button type="button" class="icon-btn" aria-label="关闭" :disabled="busy" @click="emit('close')">
          <X :size="15" />
        </button>
      </header>
      ...
      <label class="mb-3 block">
        ...
        <textarea ref="questionInput" v-model="front" class="field-textarea" :rows="2" :placeholder="localKind === 'vocabulary' ? '输入释义，例如：短暂的；转瞬即逝的' : '输入问题'" autofocus />
      </label>
      ...
      <footer class="flex justify-end gap-2">
        <button v-if="hasSource && !editingId" type="button" class="ghost-btn mr-auto" :disabled="busy" @click="emit('reselect-source', { kind: localKind, front, back, detail, example, rubric })">重新选择来源</button>
        <button type="button" class="ghost-btn" :disabled="busy" @click="emit('close')">取消</button>
        <button type="button" class="primary-btn" :disabled="busy || !front.trim() || !back.trim()" @click="submit">
          {{ editingId ? "保存修改" : "创建卡片" }}
        </button>
      </footer>
    </div>
