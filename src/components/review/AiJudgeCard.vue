...
import type { AiEvaluationResult, ReviewCard } from "../../domain/types";
...
const props = defineProps<{
  ...
  busy: boolean;
  suspended?: boolean;
  evaluate?: (cardId: string, answer: string, version: number) => Promise<AiEvaluationResult>;
}>();
...
async function submit(): Promise<void> {
  ...
  try {
    const result = props.evaluate
      ? await props.evaluate(props.card.id, answer.value, props.card.version)
      : await evaluateAnswer(props.card.id, answer.value, props.card.version, operationId);
...
function handleKeydown(event: KeyboardEvent): void {
  if (props.suspended || props.busy) return;
