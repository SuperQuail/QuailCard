import type { AdoptCardsInput, AdoptCardsResult, CardInput, DictationResult, GenerationResult, NoteCard, ReviewCard } from "../../domain/types";
...
/** 演示采纳保留同一 UUID 与展示字段；真实校验及原子写入由 Rust 实现。 */
export function adoptCards(input: AdoptCardsInput): AdoptCardsResult {
  const result: AdoptCardsResult = { addedIds: [], existingIds: [], duplicateIds: [] };
  for (const draft of input.cards) {
    const fields = draft.fields;
    if (cards.has(draft.draftId)) {
      result.existingIds.push(draft.draftId);
      continue;
    }
    const nextSequence = cardSequence + 1;
    ...
    card.id = draft.draftId;
    card.sourceRef = fields.source ?? "";
    card.source = draft.source;
    card.aliases = (fields.aliases ?? "").split(/[、,，]/).map((item) => item.trim()).filter(Boolean);
    card.rubric = (fields.rubric ?? "").split(/[、,，]/).map((item) => item.trim()).filter(Boolean);
    cards.set(card.id, card);
    setCardSequence(nextSequence);
    result.addedIds.push(card.id);
  }
  return result;
}
...
export function generateCards(input: { typeId: string; requestedCount: number }): GenerationResult {
  ...
  return { cards: drafts.slice(0, count).map((draft) => ({ ...draft, draftId: crypto.randomUUID(), source: null })), warnings: ["浏览器演示使用固定示例草稿。"] };
}
