...
export function adoptCards(input: AdoptCardsInput): Promise<AdoptCardsResult> {
  return call<AdoptCardsResult>("adopt_cards", { input });
}
...
/** 登记生成任务，后续使用任务 ID 查询或停止。 */
export function startGeneration(input: GenerationInput): Promise<GenerationTaskStart> {
  return call<GenerationTaskStart>("start_generation", { input });
}

/** 获取后端真实阶段及终态草稿。 */
export function getGenerationStatus(taskId: string): Promise<GenerationTaskStatus> {
  return call<GenerationTaskStatus>("get_generation_status", { taskId });
}

/** 停止请求保留后端已校验通过的草稿。 */
export function cancelGeneration(taskId: string): Promise<GenerationTaskStatus> {
  return call<GenerationTaskStatus>("cancel_generation", { taskId });
}
